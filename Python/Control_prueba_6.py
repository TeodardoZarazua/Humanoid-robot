...
def detection_thread():
    global latest_left_code, latest_left_name
    global latest_right_code, latest_right_name
    global manual_mode, freeze_left, freeze_right
    ...
    def detect_arm_positions_side(lm, side="left"):
        ...
        # 1) IZQUIERDA / DERECHA CON PRIORIDAD
        if side == "left":
            if dx > 0.18:
                return "3", "Izquierda"
            if dx < -0.18:
        ...
        # 2) ARRIBA
        if dy < -0.12 and abs(dx) < 0.15:
            return "2", "Arriba"

        # 3) ATRÁS MENOS SENSIBLE
        if side == "right":
            if dy > 0.28 and abs(dx) < 0.10:
                return "1", "Quieto"   # lo que antes era atrás
        else:
            if dy > 0.20 and abs(dx) < 0.12:
                return "1", "Quieto"
    ...
    with mp_pose.Pose(
        ...
        while running:
            ...
            if results.pose_landmarks:
                ...
                detL, detNameL = detect_arm_positions_side(lm, "left")
                detR, detNameR = detect_arm_positions_side(lm, "right")

                # =======================
                # FREEZE RULES CRUZADOS
                # =======================
                # 1) Si el brazo derecho hace Izquierda → freeze izquierdo
                if detR == "3":
                    freeze_left = True
                else:
                    freeze_left = False

                # 2) Si el brazo izquierdo hace Derecha → freeze derecho
                if detL == "4":
                    freeze_right = True
                else:
                    freeze_right = False

                # Aplicar freeze izquierdo
                if freeze_left:
                    codeL = "1"
                    nameL = "Quieto"
                else:
                ...
                # =======================
                # GESTO A/B AUTOMÁTICOS
                # =======================
                # Gesto A: ambos brazos arriba → "A"
                if codeL == "2" and codeR == "2":
                    codeL = "A"
                    codeR = "A"
                    nameL = "Gesto A"
                    nameR = "Gesto A"

                # Gesto B: ambos brazos atrás → "B"
                if codeL == "5" and codeR == "5":
                    codeL = "B"
                    codeR = "B"
                    nameL = "Gesto B"
                    nameR = "Gesto B"
